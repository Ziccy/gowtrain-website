...
export default function TrainersPage() {
  ...
  const filteredAndSortedTrainers = useMemo((): Trainer[] => {
    ...
    return result.sort((a, b) => {
      ...
      // Standaard op rating (hoogst eerst)
  ...
  async function loadTrainers(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        ...
        setErrorMessage("De trainers konden niet worden geladen. Probeer het opnieuw.");
        setTrainers([]);
        return;
      }
      ...
      <section className="relative flex-1 overflow-hidden py-12 sm:py-16 lg:py-20">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
          ...
          <div className="max-w-4xl">
            <p className="font-display text-lg text-[#FF4B3E]">VIND JOUW MATCH</p>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[#D7D9DA] sm:text-xl">
              Zoek op stad, sport of specialisatie. Bekijk beschikbare trainers en boeken maar: Gow!
            </p>
          </div>
          ...
          <div className="mt-10 border-2 border-white bg-white p-3 text-[#14171A] shadow-[8px_8px_0_0_#FF4B3E]">
            <div className="bg-[#14171A] p-4 sm:p-6">
              ...
              {/* Sport & Sorteer opties */}
              <div className="mt-6 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="font-display text-xs text-[#D6FF3F]">SPORT</p>
                ...
                {/* Sorteren */}
                <div>
                  <p className="font-display text-xs text-[#D6FF3F]">SORTEER OP</p>
                  <select
                    value={sortBy}
          ...
          {!loading && (
            <div className="mt-10 flex flex-col justify-between gap-4 border-b-2 border-white/20 pb-5 sm:flex-row sm:items-end">
              <div>
                <p className="font-display text-lg text-[#FF4B3E]">BESCHIKBARE TRAINERS</p>
                <h2 className="mt-2 font-display text-4xl sm:text-5xl">
                  {filteredAndSortedTrainers.length}{" "}
                  {filteredAndSortedTrainers.length === 1 ? "MATCH." : "MATCHES."}
                </h2>
              </div>
          ...
          {/* Laden met geanimeerde merkstijl */}
          {loading && (
          ...
          {/* Geen zoekresultaten */}
          {!loading && !errorMessage && filteredAndSortedTrainers.length === 0 && (
            <section className="mt-8 border-2 border-white bg-white p-3 text-[#14171A] shadow-[8px_8px_0_0_#D6FF3F]">
              <div className="bg-[#14171A] p-6 text-white sm:p-8">
                <p className="font-display text-4xl text-[#D6FF3F]">NOG GEEN MATCH.</p>
                <p className="mt-4 max-w-xl text-lg leading-relaxed text-[#B9BEC2]">
                  We vonden geen trainer die bij deze zoekopdracht past. Pas je zoekterm of sportfilter aan.
                </p>
                <button
                  type="button"
                  onClick={() => {
          ...
          {!loading && !errorMessage && filteredAndSortedTrainers.length > 0 && (
            <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredAndSortedTrainers.map((trainer: Trainer) => {
                const hasRating =
                  trainer.rating !== null &&
                ...
                return (
                  ...
                  >
                    <div className="flex h-full flex-col justify-between bg-[#14171A] p-5 text-white">
                      ...
                      <div>
                        {/* Top rij: Profielfoto & Naam + Badges uit Brandbook Pagina 13 */}
                        ...
                        {/* Badges: Rating + Locatie direct naast elkaar */}
                        <div className="mt-4 flex items-center gap-2">
                          ...
                          <span className="bg-[#2A2E31] px-2.5 py-1 font-display text-xs text-white truncate max-w-[200px]">
                        ...
                        {/* Focus */}
                        <div className="mt-6">
                          <p className="font-display text-xs text-[#FF4B3E]">SPECIALISATIE</p>
                        ...
                        {/* Bio */}
                        <p className="mt-3 min-h-16 text-sm leading-relaxed text-[#B9BEC2] line-clamp-3">
                          {trainer.bio
                            ? trainer.bio
                      ...
                      {/* Prijs & CTA Knop conform Brandbook Pagina 13 ("GOW!") */}
                      <div className="mt-6 border-t border-white/20 pt-4">
                        <div className="flex items-end justify-between">
                          <div>
                            <span className="block font-display text-xs text-[#8A8F94]">PRIJS PER UUR</span>
                            <span className="font-display text-4xl text-[#D6FF3F]">
                              {formatPrice(trainer.price_per_hour)}
                            </span>
                          </div>

                          {/* CTA Knop met Gow! merknaam */}
                          <Link
